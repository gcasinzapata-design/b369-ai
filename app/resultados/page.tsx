  ...
  const buscar = async ()=>{
    ...
    try{
      const res = await fetch('/api/search', {
        ...
        body: JSON.stringify({
          q,
          filtros:{ distrito, areaMin:Number(areaMin), habMin:Number(habMin), precioMax:Number(precioMax) }
        })
      })
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <div className="space-y-3">
        <div className="card p-3 grid md:grid-cols-5 gap-2">
          ...
          <input className="input" type="number" value={precioMax} onChange={e=>setPrecioMax(Number(e.target.value))} placeholder="Precio max"/>
