...
export default function Tasador(){
  const [form,setForm] = useState({
    ...
    areaTerreno_m2: 0,                  // Opcional (relevante para casas)
    antiguedad_anos:8,
    vista_mar:true,
    habitaciones:2,
    banos:2,
  ...
  const calc = async ()=>{
    ...
    if (!form.areaConstruida_m2 || form.areaConstruida_m2 <= 0) {
      setError('El campo "Área construida (m²)" es obligatorio y debe ser mayor a 0.')
      return
    }
    if (!form.direccion || form.direccion.trim().length < 4) {
      ...
      <div className="card p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <label className="label">Área construida (m²)*
            <input className="input" type="number" value={form.areaConstruida_m2}
              onChange={e=>setForm({...form, areaConstruida_m2:Number(e.target.value)})}/>
          </label>

          <label className="label">Área de terreno (m²)
            <input className="input" type="number" value={form.areaTerreno_m2}
              onChange={e=>setForm({...form, areaTerreno_m2:Number(e.target.value)})}/>
          </label>
          ...
          <label className="label">Estac.
            <input className="input" type="number" value={form.estacionamientos}
              onChange={e=>setForm({...form, estacionamientos:Number(e.target.value)})}/>
          </label>

          <label className="label">Antigüedad (años)
            <input className="input" type="number" value={form.antiguedad_anos}
          ...
          {out && (
            <div className="space-y-3 text-sm">
              <div className="flex flex-wrap gap-2">
                ...
                {out.percentiles && (
                  <span className="badge">P25/P50/P75: ${out.percentiles.p25.toLocaleString()} / ${out.percentiles.p50.toLocaleString()} / ${out.percentiles.p75.toLocaleString()}</span>
                )}
              ...
              <ul className="list-disc pl-5 space-y-1">
                {out.comparables.map((c, i)=>(
                  <li key={i}>
                    ...
                    {c.direccion ? ` · ${c.direccion}` : ''} {c.url ? <a className="text-blue-600 ml-1" href={c.url} target="_blank">Ver →</a> : null}
                  </li>
                ))}
              </ul>
            </div>
